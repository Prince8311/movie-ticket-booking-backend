...
if ($requestMethod == 'GET') {
    ...
    if (isset($_GET['theaterName']) && isset($_GET['screen']) && isset($_GET['screenId']) && isset($_GET['section'])) {
        ...
        $sql = "SELECT `id`, `section`, `row` FROM `screen_sections` WHERE `theater_name`='$theaterName' AND `screen`='$screen' AND `screen_id`='$screenId' AND `section`='$section'";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            $rowData = mysqli_fetch_assoc($result);
            $noOfRows = $rowData['row'];

            $data = [
                'status' => 200,
                'message' => 'No of rows fetched.',
                'noOfRows' => $noOfRows
            ];
