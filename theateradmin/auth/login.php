...
if ($requestMethod == 'POST') {
    ...
    if (!empty($inputData)) {
        ...
        $sql = "SELECT * FROM `theater_users` WHERE `name`='$user' OR `email`='$user' OR `phone`='$user'";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                if (password_verify($password, $row['password'])) {
                    ...
                    try {
                                                ...
                                                <div class="template" style="position: relative; background: #FFF; padding-bottom: 50px; border-radius: 5px;" >
                                                    ...
                                                    <div class="body_message" style="position: relative; margin-top: 15px;">
                                                        <p style="position: relative; text-align: center;">
                                                            <span style="position: relative; text-align: center; font-family: sans-serif; color: #444; font-size: 15px; line-height: 1.4;">Your OTP for authentication in <a href="theateradmin.ticketbay.in" style="color: #FC6736;" >theateradmin.ticketbay.in</a> is,</span>
                                                        </p>
                                                    </div>
                        ...
                        $mail->send();
                        $updateSql = "UPDATE `theater_users` SET `mail_otp`='$otp' WHERE `id` = '$userId'";
