...
if ($requestMethod == 'GET') {
    ...
    if (isset($_GET['theaterName']) && isset($_GET['screen']) && isset($_GET['screenId'])) {
        ...
        $sql = "SELECT `section_name` FROM `screen_sections` WHERE `theater_name`='$theaterName' AND `screen`='$screen' AND `screen_id`='$screenId'";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            $sectionData = mysqli_fetch_all($result, MYSQLI_ASSOC);
